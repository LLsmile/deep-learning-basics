...
datasets_ = '/media/ssd4t/liaolin/01_box/truck_test/3'
cropped_dir = '/media/ssd4t/liaolin/01_box/truck_test/box_XYZ_PJ23_3001_1_20240109'

datasets_ = '/media/ssd4t/liaolin/01_box/projects/6-1_20times'
cropped_dir = '/media/ssd4t/liaolin/01_box/projects/6-1_20times_contrast_for_seg'

datasets_ = '/media/ssd4t/liaolin/01_box/projects/milk_10times'
cropped_dir = '/media/ssd4t/liaolin/01_box/projects/milk_10times_for_seg'

datasets_ = '/media/ssd4t/liaolin/01_box/projects/6-1_yolox'
cropped_dir = '/media/ssd4t/liaolin/01_box/projects/6-1_yolox_for_Seg'

#datasets_ = '/home/liaolin/xyz-dataset-tools/truck_depalletize/train'
cropped_dir ='/media/ssd4t/liaolin/01_box/projects/truck_train_for_seg_test'

datasets_ = '/media/ssd4t/xinzhedeng/datasets/box30'
cropped_dir = '/media/ssd4t/liaolin/01_box/truck/standard_train2'
#datasets_ = '/media/ssd4t/liaolin/01_box/truck/train_xyz_format'

#datasets_ = '/media/ssd4t/liaolin/01_box/truck/truck+depalletize'
#cropped_dir = '/media/ssd4t/liaolin/01_box/truck/truck+depalletize_for_seg'
#datasets_ = '/media/ssd4t/liaolin/01_box/truck_test/contrast/test/3'

#cropped_dir = '/media/ssd4t/liaolin/01_box/truck_test/contrast/coco/box_maersk_shenlilu_20231215'
...
for root, dirs, filenames in os.walk(datasets_):
    for dir in dirs:
        ...
        if not os.path.exists(samples):
            samples = os.path.join(datasets, 'samples_v2.txt')
        s = open(samples, 'r')
        ...
        for sample in tqdm(samples_data):
            image_name_id += 1
            #rgb_, _, anno_ = 
            all_data = sample.strip().split(',')
            rgb_, _, anno_ = all_data[:3]
            ...
            if not os.path.exists(os.path.join(datasets, rgb_)):
                continue
            ...
            for instance in instances:
                ...
                anno_id += 1
                '''
                seg_inst = img[int(y1):int(y2), int(x1):int(x2)]
                seg_inst = cv2.resize(seg_inst, (160,160))
                cv2.imwrite(os.path.join(cropped_root, str(anno_id) + ".png"), seg_inst)
                '''
            ...
            if 1:
                image_name_id += 1
                dumplicate = str(image_name_id).zfill(8) + '.png'
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                cv2.imwrite(os.path.join(cropped_root, dumplicate), img)
                src = os.path.join(txt_root, str(image_name_id-1).zfill(8)+".txt")
                tar = os.path.join(txt_root, str(image_name_id).zfill(8)+".txt")
                shutil.copy(src, tar)
            image_info = {
