...
def predict_image(infer_config, predictor, img_list):
    ...
    for img_path in img_list:
        ...
        for key in inputs.keys():
            print(f"keys:{key} inputs: {inputs[key]}")
        '''
        ...
        for bbox in bboxes:
            if bbox[0] > -1 and bbox[1] > infer_config['draw_threshold']:
                points = [bbox[2], bbox[3], bbox[4], bbox[5], bbox[6], bbox[7],bbox[8],bbox[9]]
                points = list(map(int, points))
                for i in range(4):
                    j =(i + 1) % 4
                    cv2.line(img, (points[2*i], points[2*i+1]), (points[2*j],points[2*j+1]), (0,0,255), 3)
        ...
        for i in range(8400):
            ...
            if scores[i] > 0.5:
                '''
                rect = ((boxes[0], boxes[1]), (boxes[2], boxes[3]), (boxes[4]))
                box = cv2.boxPoints(rect)
                box = np.int0(box)
                '''
                points = obb2poly_le135(torch.Tensor([boxes[i]]))

                #print(points)
                #img = cv2.rectangle(img, (boxes[i][0]-boxes[i][2]/2, boxes[i][1] - boxes[i][3]/2), (boxes[i][0]+boxes[i][2]/2, boxes[i][1] + boxes[i][3]/2), (0,0,255), 2)
                points = np.int0(points)[0]
                for i in range(4):
                    j =(i + 1) % 4
                    cv2.line(img, (points[2*i], points[2*i+1]), (points[2*j],points[2*j+1]), (0,0,255), 3)
                #img = cv2.drawContours(img, box, 0, (0,0,255), 2)
        cv2.imwrite("viz.png", img)
        print(outputs[0].shape)
        print(outputs[1].shape)
        for key in inputs.keys():
            print(f"keys:{key} inputs: {inputs[key]}")
